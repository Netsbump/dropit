...
export class PersonalRecordUseCases implements IPersonalRecordUseCases {
  ...
  async getAll(
    ...
    if (isUserCoach) {
      ...
      personalRecords =
        await this.personalRecordRepository.getAll(athleteUserIds);
      ...
      personalRecords =
        await this.personalRecordRepository.getAllByAthleteId(currentUserId);
  ...
  async getAllByAthleteId(
    ...
    const personalRecords =
      await this.personalRecordRepository.getAllByAthleteId(athleteId);
  ...
  async getAllPersonalRecordsSummaryByAthleteId(
    athleteId: string,
    currentUserId: string,
    organizationId: string
    ...
    const cleanAndJerkRecords = personalRecords.filter(
    ...
    if (snatchRecords.length > 0) {
      summary.snatch = Math.max(
        ...snatchRecords.map((r: PersonalRecord) => r.weight)
      );
    }
  ...
  async create(
    ...
    const createdPersonalRecord = await this.personalRecordRepository.getOne(
      personalRecord.id
    );
    if (!createdPersonalRecord) {
      throw new PersonalRecordNotFoundException(
        'Personal record not found after creation'
      );
    }
  ...
  async update(
    ...
    const updatedPersonalRecord =
      await this.personalRecordRepository.getOne(id);
    if (!updatedPersonalRecord) {
      throw new PersonalRecordNotFoundException(
        'Personal record not found after update'
      );
    }
