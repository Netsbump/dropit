...
const { Input, Confirm, Select } = Enquirer as unknown as {
  Input: new (options: { message: string; initial?: string }) => {
    run: () => Promise<string>;
  };
  Confirm: new (options: {
    name: string;
...
function runCommand(command: string, args: string[]): Promise<void> {
  return new Promise((resolve, reject) => {
    console.log(
      `\n  ${colorize('→', 'cyan')} Running: ${colorize(
        `${command} ${args.join(' ')}`,
        'dim'
      )}\n`
    );
...
async function waitForDatabase(
  ...
  console.log(
    `  ${colorize('⏳', 'yellow')} Waiting for database to be ready...`
  );
  ...
  for (let i = 0; i < maxRetries; i++) {
    ...
    process.stdout.write(
      `  ${colorize('⏳', 'yellow')} Waiting... (${i + 1}/${maxRetries})\r`
    );
  }
...
function getMissingVariables(examplePath: string, envPath: string): string[] {
  ...
  return Object.keys(exampleVars).filter(
...
function updateEnvFile(
  ...
  if (!existsSync(filePath)) {
    console.log(
      `  ${colorize('⚠', 'yellow')} File not found: ${colorize(
        filePath,
        'dim'
      )}`
    );
...
function copyEnvFiles(envFilesInfo: EnvFileInfo[]): void {
  ...
  for (const { from, to, exists, missingVars } of envFilesInfo) {
      ...
      if (missingVars.length > 0) {
        console.log(
          `  ${colorize('⚠', 'yellow')} ${colorize(
            to,
            'dim'
          )} exists but missing variables: ${colorize(
            missingVars.join(', '),
            'yellow'
          )}`
        );
    ...
    if (existsSync(fromPath)) {
      ...
      console.log(
        `  ${colorize('✓', 'green')} Copied ${colorize(
          from,
          'dim'
        )} → ${colorize(to, 'dim')}`
      );
      ...
      console.log(
        `  ${colorize('⚠', 'yellow')} File not found: ${colorize(from, 'dim')}`
      );
    }
  }
}
...
async function promptDatabaseConfig(): Promise<EnvConfig['database']> {
  ...
  if (!needsDbConfig) {
    ...
    console.log(
      `  ${colorize('✓', 'green')} Database variables already configured`
    );
    return {
  ...
  const user = missingVars.includes('DB_USER')
    ? await prompt(
        'Database user',
        existingVars.DB_USER || exampleVars.DB_USER || 'postgres'
      )
  ...
  const name = missingVars.includes('DB_NAME')
    ? await prompt(
        'Database name',
        existingVars.DB_NAME || exampleVars.DB_NAME || 'dropit'
      )
  ...
  const host = missingVars.includes('DB_HOST')
    ? await prompt(
        'Database host',
        existingVars.DB_HOST || exampleVars.DB_HOST || 'localhost'
      )
  ...
  const portStr = missingVars.includes('DB_PORT')
    ? await prompt(
        'Database port',
        existingVars.DB_PORT || exampleVars.DB_PORT || '5432'
      )
...
async function promptPortsConfig(): Promise<EnvConfig['ports']> {
  ...
  const apiPortStr = missingVars.includes('API_PORT')
    ? await prompt(
        'API port',
        existingVars.API_PORT || exampleVars.API_PORT || '3000'
      )
  ...
  const maildevSmtpPortStr = missingVars.includes('MAILDEV_SMTP_PORT')
    ? await prompt(
        'MailDev SMTP port',
        existingVars.MAILDEV_SMTP_PORT ||
          exampleVars.MAILDEV_SMTP_PORT ||
          '1025'
      )
    : existingVars.MAILDEV_SMTP_PORT || '1025';
...
async function promptLocalIpForMobile(apiPort: number): Promise<string> {
  ...
  if (
    ...
    console.log(
      `  ${colorize('✓', 'green')} Mobile API URL already configured`
    );
  ...
  console.log(
    `  ${colorize(
      'ℹ',
      'blue'
    )} The mobile app needs your local IP address to connect to the API`
  );

  const detectedIp = getLocalIpAddress();
  if (detectedIp) {
    console.log(
      `  ${colorize('✓', 'green')} Detected IP: ${colorize(
        detectedIp,
        'bright'
      )}`
    );
  ...
  console.log(
    `  ${colorize(
      '⚠',
      'yellow'
    )} Could not detect local IP automatically. Please enter manually.`
  );
  const localIp = await prompt(
    'Local IP address for mobile app',
    '192.168.1.1'
  );
  return localIp;
}
...
function updateAllEnvFiles(config: EnvConfig): void {
  ...
  console.log(
    `  ${colorize('✓', 'green')} Updated ${colorize('apps/api/.env', 'dim')}`
  );
  ...
  console.log(
    `  ${colorize('✓', 'green')} Updated ${colorize('apps/web/.env', 'dim')}`
  );
  ...
  console.log(
    `  ${colorize('✓', 'green')} Updated ${colorize('apps/mobile/.env', 'dim')}`
  );
}
...
async function main(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `  ${colorize('Database:', 'bright')} ${colorize(
        `${config.database.user}@${config.database.host}:${config.database.port}/${config.database.name}`,
        'dim'
      )}`
    );
    console.log(
      `  ${colorize('API:', 'bright')} ${colorize(
        `http://localhost:${config.ports.api}`,
        'blue'
      )}`
    );
    console.log(
      `  ${colorize('Web:', 'bright')} ${colorize(
        'http://localhost:5173',
        'blue'
      )} ${colorize('(Vite default)', 'dim')}`
    );
    console.log(
      `  ${colorize('Mobile:', 'bright')} ${colorize(
        `http://${config.mobile.localIp}:${config.ports.api}`,
        'blue'
      )}`
    );
    console.log(
      `  ${colorize('MailDev:', 'bright')} ${colorize(
        `http://localhost:${config.ports.maildevWeb}`,
        'blue'
      )} ${colorize(`(SMTP: ${config.ports.maildevSmtp})`, 'dim')}`
    );
    console.log(
      `  ${colorize('PgAdmin:', 'bright')} ${colorize(
        `http://localhost:${config.ports.pgadmin}`,
        'blue'
      )}`
    );
    ...
    const shouldStartDocker = await confirm(
      'Start Docker services (database, maildev, pgadmin)?'
    );
    ...
    if (shouldStartDocker) {
      try {
        ...
        if (dbReady) {
          ...
          console.log(
            `  ${colorize('→', 'cyan')} Checking for pending migrations...`
          );
          ...
          try {
            ...
            hasPendingMigrations =
              outputStr.includes('pending') || checkExitCode !== 0;
          } catch {
          ...
          if (hasPendingMigrations) {
            console.log(
              `  ${colorize('ℹ', 'blue')} Pending migrations detected`
            );
            ...
            if (dbChoice.includes('Apply migrations')) {
              try {
                ...
                console.error(
                  `\n  ${colorize('⚠', 'yellow')} Migration failed:`,
                  error
                );
                console.log(
                  `  ${colorize(
                    ...
                    'dim'
                  )} ${colorize('pnpm --filter api db:migration:up', 'bright')}`
                );
              }
              ...
              try {
                ...
                console.log(
                  `  ${colorize(
                    ...
                    'pnpm --filter api db:create && pnpm --filter api db:seed',
                    'bright'
                  )}`
                );
              }
              ...
              console.log(
                `  ${colorize(
                  '→',
                  'cyan'
                )} Skipped database setup. Run manually with: ${colorize(
                  'pnpm --filter api db:create && pnpm --filter api db:seed',
                  'bright'
                )} or ${colorize(
            ...
            const shouldSeed = await confirm(
              'Run database seeder to add test data?'
            );
            ...
            if (shouldSeed) {
              try {
                ...
                console.error(
                  `\n  ${colorize('⚠', 'yellow')} Seeding failed:`,
                  error
                );
              }
            }
          ...
          console.log(
            `  ${colorize(
              '→',
              'cyan'
            )} Database not ready. Run migrations manually with: ${colorize(
              'pnpm --filter api db:migration:up',
              'bright'
            )}`
          );
        ...
        console.error(
          `\n  ${colorize('⚠', 'yellow')} Failed to start Docker:`,
          error
        );
        console.log(
          `  ${colorize(
            'You can start Docker manually with:',
            'dim'
          )} ${colorize('docker compose up -d', 'bright')}`
        );
      }
    } else {
      console.log(
        `  ${colorize(
          '→',
          'cyan'
        )} Skipped Docker. Start manually with: ${colorize(
          'docker compose up -d',
          'bright'
        )}`
      );
    ...
    if (!dockerStarted) {
      console.log(
        `  ${colorize(`${step}.`, 'bright')} Start Docker services: ${colorize(
          'docker compose up -d',
          'blue'
        )}`
      );
      step++;
      console.log(
        `  ${colorize(`${step}.`, 'bright')} Run database setup: ${colorize(
          'pnpm --filter api db:create && pnpm --filter api db:seed',
          'blue'
        )}`
      );
    ...
    console.log(
      `  ${colorize(`${step}.`, 'bright')} Start development: ${colorize(
        'pnpm dev',
        'blue'
      )}\n`
    );
