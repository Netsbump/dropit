...
La dernière étape produit l'image finale en copiant uniquement les artefacts de production depuis l'étape de construction. L'image résultante ne contient ni les outils de développement ni le code source TypeScript, seulement le JavaScript compilé et les dépendances runtime. La configuration MikroORM est adaptée pour fonctionner avec les fichiers JavaScript compilés plutôt qu'avec TypeScript, et l'application démarre en appliquant automatiquement les migrations en attente avant de lancer le serveur.

**Gestion des données de démonstration :** Le seeding n'est pas exécuté automatiquement au démarrage de l'API en production. Pour les environnements de staging ou de démo, les seeders MikroORM peuvent être lancés explicitement (`pnpm db:seed:prod`). Les seeders de référence sont idempotents et peuvent être relancés sans dupliquer les données de base.
